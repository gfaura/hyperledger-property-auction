...
const myChaincodeName = 'PropertyAuction'; //Name of our chaincode will be PropertyAuction
...
async function submitBid(ccp,wallet,user,auctionID,bidID){
    try {
      ...
      await gateway.connect(ccp,
        ...
        console.log('\n--> Evaluate Bid: query the bid you want to submit');
        let bidString = await contract.evaluateTransaction('QueryBid',auctionID,bidID);
        console.log('*** Result: Bid: ' + prettyJSONString(bidString.toString()));

        // GF: Getting the bid amount to pass to submitBid and update the highest bid price
        var obj = JSON.parse(bidString);
        var bidAmount = obj.price;
        console.log('\n*** Bid Amount (parsed) is: $' + bidAmount);
        ...
        // GF: Adding the parameter bidAmount
        console.log('\n--> Submit Transaction: add bid to the auction');
        await statefulTxn.submit(auctionID,bidID,bidAmount);
