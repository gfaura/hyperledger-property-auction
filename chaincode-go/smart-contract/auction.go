...
type Auction struct {
	...
	CommForOrg   string             `json:"commissionorg"`
}
...
// GF: Adding asking price
func (s *SmartContract) CreateAuction(ctx contractapi.TransactionContextInterface, auctionID string, itemsold string, sellingprice int) error {
	...
	auction := Auction{
		...
		Winner:       "",
		Status:       "for sale",   // GF: Property status
		AskingPrice:  sellingprice, // GF: Adding asking price
		HighestBid:   0,
		CommForOrg:   "",
	}
...
func (s *SmartContract) SubmitBid(ctx contractapi.TransactionContextInterface, auctionID string, txID string, bidAmount int) error {
	...
	if Status != "for sale" { // GF: Updated property status
		return fmt.Errorf("cannot bid on sale agreed or sold properties")
	}
	...
	// *************************************************************************
	// GF: Validate that new bid is higher than previous bids and asking price
	if bidAmount < auctionJSON.AskingPrice {
		return fmt.Errorf("failed submitting the bid. The price offered must be greater or equal to the asking price: %v", auctionJSON.AskingPrice)
	}
	// GF: Update the highest bid if the bid submitted is higher
	if bidAmount > auctionJSON.HighestBid {
		auctionJSON.HighestBid = bidAmount
	} else {
		return fmt.Errorf("failed submitting the bid. The price offered must be greater than the existing highest bid: %v", auctionJSON.HighestBid)
	}
	// *************************************************************************
	...
	// get the auction from state again
	auctionBytes, err = ctx.GetStub().GetState(auctionID)
...
func (s *SmartContract) RevealBid(ctx contractapi.TransactionContextInterface, auctionID string, txID string) error {
	...
	Status := auctionJSON.Status
	if Status != "sale agreed" {
		return fmt.Errorf("cannot reveal bid for properties sold or for sale")
	}
...
func (s *SmartContract) CloseAuction(ctx contractapi.TransactionContextInterface, auctionID string) error {
	...
	Status := auctionJSON.Status
	if Status != "for sale" { //GF: Check status before updating
		return fmt.Errorf("cannot mark a property as Sale agreed if it is not for sale")
	}

	//Flag property as sale agreed
	auctionJSON.Status = string("sale agreed") //GF: set correct status
...
func (s *SmartContract) EndAuction(ctx contractapi.TransactionContextInterface, auctionID string) error {
	...
	Status := auctionJSON.Status
	if Status != "sale agreed" { //GF: Check status before updating
		return fmt.Errorf("Can only end a sale agreed auction")
	}
	...
	for _, bid := range revealedBidMap {
		if bid.Price > auctionJSON.Price {
			...
			auctionJSON.CommForOrg = bid.Org // GF: Added organisation that will receive the commission
		}
	}
	...
	auctionJSON.Status = string("sold") //GF: set correct status
...
// RevealBid is used by a bidder to reveal their bid after the auction is closed
func (s *SmartContract) GetBidPrice(ctx contractapi.TransactionContextInterface, auctionID string, txID string) (int, error) {

	err := verifyClientOrgMatchesPeerOrg(ctx)
	if err != nil {
		return 0, fmt.Errorf("failed to get implicit collection name: %v", err)
	}
	...
	if err != nil {
		return 0, fmt.Errorf("failed to get bid %v: %v", bidKey, err)
	}
	if bidJSON == nil {
		return 0, fmt.Errorf("bid %v does not exist", bidKey)
	}
	...
	if bid.Bidder != clientID {
		return 0, fmt.Errorf("Permission denied, client id %v is not the owner of the bid", clientID)
	}
